...
const refSchema = z.custom<ConditionRefType>((val) => typeof val === "string" && val.includes("."))
...
const conditionSchema: z.ZodType<LeafType | NotType | AndType | OrType> = z.lazy(() => z.discriminatedUnion('type', [
    leafSchema,
    // not schema
    z.object({
        type: z.literal("not"),
...
type ConditionRefType = `user.${keyof UserType}` |
