...
function Card({
  ...
  size?: "default" | "sm"
  variant?: 'success' | 'destructive' | 'warn'
}) {
  return (
    <div
      data-slot="card"
      ...
      className={cn(
        "group/card flex flex-col gap-4 overflow-hidden rounded-none bg-card py-4 text-xs/relaxed text-card-foreground ring-1 ring-foreground/10 has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-2 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-none *:[img:last-child]:rounded-none",
        {
          ...
          'bg-amber-500/10 text-amber-500': variant === 'warn'
        },
        className
      )}
      {...props}
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-xs/relaxed text-muted-foreground", 'group-data-[variant=success]/card:text-emerald-500 group-data-[variant=destructive]/card:text-destructive-500', className)}
      {...props}
    />
  )
}
